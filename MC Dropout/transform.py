...
class Compose(object):
    ...
    def __call__(self, pet, ct, label):
        for t in self.segtransform:
            pet, ct, label = t(pet, ct, label)
        return pet, ct, label
...
class ToTensor(object):
    ...
    def __call__(self, pet, ct, label):
        if not isinstance(pet, np.ndarray) or not isinstance(label, np.ndarray):
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray"
                                "[eg: data readed by cv2.imread()].\n"))
        if len(pet.shape) > 3 or len(pet.shape) < 2:
        ...
        if not isinstance(pet, torch.FloatTensor):
            pet = pet.float()
        if not isinstance(ct, torch.FloatTensor):
            ct = ct.float()
...
class Normalize(object):
    ...
    def __call__(self, pet, ct, label):
        # if self.std is None:
        #     for t, m in zip(pet, self.mean):
        #         t.sub_(m)
        # else:
        #     for t, m, s in zip(pet, self.mean, self.std):
        #         t.sub_(m).div_(s)
        return pet, ct, label
...
class Resize(object):
    ...
    def __call__(self, pet, ct, label):
        pet = cv2.resize(pet, self.size[::-1], interpolation=cv2.INTER_LINEAR)
        ct = cv2.resize(ct, self.size[::-1], interpolation=cv2.INTER_LINEAR)
...
class RandScale(object):
    ...
    def __init__(self, scale, aspect_ratio=None):
        ...
        if len(scale) == 2 \
                ...
                and 0 < scale[0] <= scale[1]:  #isinstance(scale, collections.Iterable) and
            self.scale = scale
        else:
    ...
    def __call__(self, pet, ct, label):
        ...
        pet = cv2.resize(pet, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_LINEAR)
        ct = cv2.resize(ct, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_LINEAR)
...
class Crop(object):
    ...
    """
    def __init__(self, size, crop_type='center', padding=None, ignore_label=21):
    ...
    def __call__(self, pet, ct, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            pet = cv2.copyMakeBorder(pet, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half,
                                     cv2.BORDER_CONSTANT, value=self.padding)
            ct = cv2.copyMakeBorder(ct, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half,
                                     cv2.BORDER_CONSTANT, value=self.padding)
        ...
        pet = pet[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        ct = ct[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w]
...
class RandRotate(object):
    ...
    def __init__(self, rotate, padding, ignore_label=21, p=0.5):
    ...
    def __call__(self, pet, ct, label):
        if random.random() < self.p:
            ...
            pet = cv2.warpAffine(pet, matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                                 borderValue=self.padding)
            ct = cv2.warpAffine(ct, matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                                 borderValue=self.padding)
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, pet, ct, label):
        if random.random() < 0.5:
            pet = cv2.GaussianBlur(pet, (self.radius, self.radius), 0)
            ct = cv2.GaussianBlur(ct, (self.radius, self.radius), 0)
        return pet, ct, label
...
class RGB2BGR(object):
    ...
    def __call__(self, pet, ct, label):
        pet = cv2.cvtColor(pet, cv2.COLOR_RGB2BGR)
        ct = cv2.cvtColor(ct, cv2.COLOR_RGB2BGR)
        return pet, ct, label
...
class BGR2RGB(object):
    ...
    def __call__(self, pet, ct, label):
        ...
        ct = cv2.cvtColor(ct, cv2.COLOR_BGR2RGB)
        return pet, ct, label
