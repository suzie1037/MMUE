...
def train_epoch(dataloader, model, optimizer, epoch, args):
    ...
    for i, batch in enumerate(dataloader):
        ...
        dice += dice_value
        if i % 100 == 0:
            print("\tSeg Loss: %.3f, Dice: %.3f  Std: %.3f "% (seg_loss.float(), dice_value, model.std.mean().float().item()))
...
def test_epoch(dataloader, model):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            output_tensor, prob = model(pet_tensor, ct_tensor, dropout=False)  # bs, 2, 80, 80
            pred_tensor = torch.argmax(output_tensor, 1)
