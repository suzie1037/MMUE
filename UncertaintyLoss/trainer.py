...
def train_epoch(dataloader, model, optimizer, epoch, args):
    ...
    for i, batch in enumerate(dataloader):
        ...
        ### calculate uncertainty by weight averaging
        with torch.no_grad():
            _, prob_average = model(pet_tensor, ct_tensor, dropout=False)
            _, prob_t = model(pet_tensor, ct_tensor, dropout=True)
            bs, c, h, w, = prob_average.size()
            uncertainty = (prob_average - prob_t) * (prob_average - prob_t)
            uncertainty = uncertainty[:, 1, :, :]
            temp = uncertainty.reshape([bs, h * w])
            uncertainty = (uncertainty - torch.min(temp, dim=1)[0].view(bs, 1, 1)) / (
                        torch.max(temp, dim=1)[0].view(bs, 1, 1) - torch.min(temp, dim=1)[0].view(bs, 1, 1) + 1e-5)

        # ### optimize backbone
        output_tensor, prob = model(pet_tensor, ct_tensor, dropout=True)
        pred_tensor = torch.argmax(output_tensor, 1)

        ### get uncertainty weighted loss
        seg_loss = model.get_loss(output_tensor, label_tensor, uncertainty.detach(), args.theta)
        loss = seg_loss
        optimizer.zero_grad()
        loss.backward() #
        optimizer.step()
...
def train(args, dataloader, num_epochs, model, optimizer):
    ...
    ExpLR_backbone = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.98)
